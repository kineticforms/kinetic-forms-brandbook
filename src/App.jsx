...
const CanvasWave = () => {
  ...
  return (
    <canvas
      id="bg-canvas"
      ref={canvasRef}
      className="absolute inset-0 w-full h-full pointer-events-none no-print"
    />
  );
};
...
const App = () => {
  ...
  const generatePaddedSocialImage = (
    svgString,
    canvasW,
    canvasH,
    drawW,
  ...
  const triggerAssetsDownload = async () => {
    ...
    try {
      ...
      const posLogoTransparentSvg =
        `<svg ${svgAttrs} viewBox="0 0 1024 1024">` +
        `<text x="512" y="720" ${fontAttrs} fill="#000000" text-anchor="middle">K.</text></svg>`;

      const negLogoTransparentSvg =
        `<svg ${svgAttrs} viewBox="0 0 1024 1024">` +
        `<text x="512" y="720" ${fontAttrs} fill="#ffffff" text-anchor="middle">K.</text></svg>`;

      const posLogoSvg =
        `<svg ${svgAttrs} viewBox="0 0 1024 1024">` +
        `<rect x="64" y="64" width="896" height="896" fill="#000000" rx="192"/>` +
        `<text x="512" y="720" ${fontAttrs} fill="#ffffff" text-anchor="middle">K.</text></svg>`;

      const negLogoSvg =
        `<svg ${svgAttrs} viewBox="0 0 1024 1024">` +
        `<rect x="64" y="64" width="896" height="896" fill="#ffffff" rx="192"/>` +
        `<text x="512" y="720" ${fontAttrs} fill="#000000" text-anchor="middle">K.</text></svg>`;

      const posLogoTextSvg =
        `<svg ${svgAttrs} viewBox="0 0 1600 512">` +
        `<rect x="128" y="128" width="256" height="256" fill="#000000" rx="32"/>` +
        `<text x="256" y="325" ${fontAttrsSmall} fill="#ffffff" text-anchor="middle">K.</text>` +
        `<text x="480" y="310" ${fontAttrsText} fill="#000000" text-anchor="start">Kinetic Forms</text></svg>`;

      const negLogoTextSvg =
        `<svg ${svgAttrs} viewBox="0 0 1600 512">` +
        `<rect x="128" y="128" width="256" height="256" fill="#ffffff" rx="32"/>` +
        `<text x="256" y="325" ${fontAttrsSmall} fill="#000000" text-anchor="middle">K.</text>` +
        `<text x="480" y="310" ${fontAttrsText} fill="#ffffff" text-anchor="start">Kinetic Forms</text></svg>`;

      const withTextFolder = zip.folder("with-text");
      const withoutTextFolder = zip.folder("without-text");
      const logoTransparentFolder =
        withoutTextFolder.folder("logo-transparent");
      const logoFolder = withoutTextFolder.folder("logo");
      const socialAppFolder = zip.folder("social-and-app");

      logoTransparentFolder
        .folder("svg")
        .file(
          "kinetic-forms-logo-transparent-positive.svg",
          posLogoTransparentSvg,
        );
      logoTransparentFolder
        .folder("svg")
        .file(
          "kinetic-forms-logo-transparent-negative.svg",
          negLogoTransparentSvg,
        );
      logoFolder
        .folder("svg")
        .file("kinetic-forms-logo-positive.svg", posLogoSvg);
      logoFolder
        .folder("svg")
        .file("kinetic-forms-logo-negative.svg", negLogoSvg);
      withTextFolder
        .folder("svg")
        .file("kinetic-forms-logo-text-positive.svg", posLogoTextSvg);
      withTextFolder
        .folder("svg")
        .file("kinetic-forms-logo-text-negative.svg", negLogoTextSvg);
      ...
      const processAssetBatch = async (
        ...
        parentFolder
          .folder("png")
          .file(
            `${baseName}-${width}x${height}.png`,
            await generateRasterImage(svgData, width, height, "png"),
          );
        parentFolder
          .folder("webp")
          .file(
            `${baseName}-${width}x${height}.webp`,
            await generateRasterImage(svgData, width, height, "webp"),
          );
        parentFolder
          .folder("jpeg")
          .file(
            `${baseName}-${width}x${height}.jpg`,
            await generateRasterImage(svgData, width, height, "jpeg", bgColor),
          );
      };
      ...
      for (const size of markSizes) {
        await processAssetBatch(
          logoTransparentFolder,
          posLogoTransparentSvg,
          "kinetic-forms-logo-transparent-positive",
          size,
          size,
          "#ffffff",
        );
        await processAssetBatch(
          logoTransparentFolder,
          negLogoTransparentSvg,
          "kinetic-forms-logo-transparent-negative",
          size,
          size,
          "#000000",
        );
      ...
      for (const width of lockupWidths) {
        ...
        await processAssetBatch(
          withTextFolder,
          posLogoTextSvg,
          "kinetic-forms-logo-text-positive",
          width,
          height,
          "#ffffff",
        );
        await processAssetBatch(
          withTextFolder,
          negLogoTextSvg,
          "kinetic-forms-logo-text-negative",
          width,
          height,
          "#000000",
        );
      ...
      socialAppFolder.file(
        "favicon.ico",
        await generateRasterImage(posLogoSvg, 32, 32, "png"),
      );
      socialAppFolder.file(
        "apple-touch-icon.png",
        await generateRasterImage(posLogoSvg, 180, 180, "png"),
      );
      socialAppFolder.file(
        "android-chrome-192x192.png",
        await generateRasterImage(posLogoSvg, 192, 192, "png"),
      );
      socialAppFolder.file(
        "android-chrome-512x512.png",
        await generateRasterImage(posLogoSvg, 512, 512, "png"),
      );
      socialAppFolder.file(
        "app-store-icon.jpg",
        await generateRasterImage(posLogoSvg, 1024, 1024, "jpeg", "#ffffff"),
      );
      socialAppFolder.file(
        "play-store-icon.png",
        await generateRasterImage(posLogoSvg, 512, 512, "png"),
      ...
      socialAppFolder.file(
        "open-graph-image-light.jpg",
        await generatePaddedSocialImage(
          posLogoTextSvg,
          1200,
          630,
          800,
      ...
      alert(
        "Failed to generate assets. Please ensure your browser has enough memory available.",
      );
    } finally {
      setTimeout(
        () => setDownloadStatus((prev) => ({ ...prev, assets: "idle" })),
        4000,
      );
    }
  };
...
export default App;`;
      ...
      alert(
        "Failed to export source code. Please ensure your browser has enough memory available.",
      );
    } finally {
      setTimeout(
        () => setDownloadStatus((prev) => ({ ...prev, source: "idle" })),
        3000,
      );
    }
  };
            ...
            <div className="space-y-16">
              <div className="space-y-6">
                ...
                <p className="max-w-2xl text-zinc-500 text-2xl leading-relaxed tracking-normal">
                  Kinetic Forms is an AI-native design system built for
                ...
                <div className="grid grid-cols-3 gap-8">
                  <div className="p-8 border border-zinc-200 rounded-2xl bg-white">
                    ...
                    <p className="text-zinc-500 text-lg leading-relaxed tracking-normal">
                      Frictionless interactions and rapid visual feedback that
                      propel users forward at the speed of thought.
                    </p>
                  </div>
                  <div className="p-8 border border-zinc-200 rounded-2xl bg-white">
                    ...
                    <p className="text-zinc-500 text-lg leading-relaxed tracking-normal">
                      Seamless, intuitive pathways powered by AI, designed to
                      adapt naturally to human behavior.
                    </p>
                  </div>
                  <div className="p-8 border border-zinc-200 rounded-2xl bg-white">
                    ...
                    <p className="text-zinc-500 text-lg leading-relaxed tracking-normal">
                      An uncompromising attention to detail, ensuring every
                      pixel, transition, and micro-interaction is perfectly
                      resolved.
                    </p>
                  </div>
                </div>
            ...
            <div className="space-y-16">
              <div className="space-y-6 max-w-2xl">
                ...
                <p className="text-zinc-500 text-2xl leading-relaxed tracking-normal">
                  ...
                  directed energy. The Kinetic Forms identity is designed to
                  scale effortlessly, maintaining absolute clarity from a 16px
                  favicon to a stadium display.
                </p>
              </div>
              <div className="space-y-12">
                <div>
              ...
              <div className="grid grid-cols-2 gap-16 border-t border-zinc-200 pt-16">
                <div className="space-y-8">
                  ...
                  <p className="text-zinc-500 text-xl leading-relaxed tracking-normal">
                    ...
                    distinct, it ensures perfect clarity and focus, stripping
                    away cognitive load to let the content breathe.
                  </p>
                </div>
                <div className="space-y-12">
                  ...
                  <div>
                    ...
                    <p className="text-2xl text-zinc-600 leading-relaxed tracking-normal">
                      ...
                      meticulously considered. We believe that true
                      technological sophistication shouldn't feel mechanical—it
                      should feel like an extension of your own thought process.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="w-[1200px] h-[1697px] shrink-0 flex flex-col justify-center px-32 bg-[#FAFAFA]">
            <div className="space-y-16">
              <div className="space-y-6 max-w-2xl">
                ...
                <p className="text-zinc-500 text-2xl leading-relaxed tracking-normal">
                  Our palette is unapologetically minimalist. By constraining
                  our colors to high-contrast monochromes, we eliminate visual
                  noise, direct focus, and establish a premium, modern
                  aesthetic.
                </p>
              </div>
              ...
              <div className="mt-16 bg-black rounded-3xl p-24 text-white overflow-hidden relative">
                <div className="relative z-10 max-w-xl space-y-6">
                  ...
                  <p className="text-zinc-400 text-2xl leading-relaxed tracking-normal">
                    Binary precision meets bold execution. Our core visual
                    language relies on stark white against deep black,
                    reflecting our AI-native foundation while creating striking,
                    undeniable clarity.
                  </p>
                </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-900 font-sans selection:bg-zinc-200">
      ...
      >
        ...
        <section
          id="view-concept"
          className={`view-section space-y-24 animate-in fade-in duration-700 ${view === "concept" ? "block" : "hidden"}`}
        >
          <div className="space-y-6">
            <TypewriterHeading />
            ...
            <div className="aspect-square bg-white border border-zinc-200 rounded-3xl p-12 flex flex-col justify-between hover:border-zinc-300 transition-colors">
              <div className="space-y-6">
                ...
                <p className="text-zinc-500 text-base max-w-sm">
                  Subtle geometric offset patterns help give a sense of
                  intrigue, while maintaining structural integrity across all
                  digital mediums.
                </p>
              </div>
              <div className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-black">
        ...
        <section
          id="view-logo"
          className={`view-section space-y-16 animate-in fade-in duration-700 ${view === "logo" ? "block" : "hidden"}`}
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 max-w-none">
            <div className="space-y-6 max-w-2xl">
              ...
              <p className="text-zinc-500 text-lg leading-relaxed">
                ...
                effortlessly, maintaining absolute clarity from a 16px favicon
                to a stadium display.
              </p>
            </div>
            ...
            <div className="flex flex-col items-start md:items-end gap-3 shrink-0 hide-in-export">
              <button
                ...
                className={`px-6 py-3 rounded-full font-medium text-sm flex items-center gap-2 transition-all ${
                  downloadStatus.assets === "success"
                    ? "bg-green-500 text-white border-transparent"
                    : downloadStatus.assets === "downloading"
                      ? "bg-zinc-100 text-zinc-400 border-zinc-200 cursor-not-allowed"
                      : "bg-white text-black border-zinc-200 hover:border-black hover:bg-zinc-50 shadow-sm border"
                }`}
              >
                {downloadStatus.assets === "idle" && (
                  <Download className="w-4 h-4" />
                )}
                {downloadStatus.assets === "downloading" && (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ...
                {downloadStatus.assets === "idle" &&
                  "Download Complete Asset Kit (.ZIP)"}
                {downloadStatus.assets === "downloading" &&
                  "Generating Assets (10-15s)..."}
                {downloadStatus.assets === "success" &&
        ...
        <section
          id="view-typography"
          className={`view-section space-y-16 animate-in fade-in duration-700 ${view === "typography" ? "block" : "hidden"}`}
        >
          <div className="space-y-6 max-w-2xl">
            <h2 className="text-4xl md:text-5xl font-medium tracking-tight">
              Typography
            ...
            <div className="space-y-10">
              ...
              <div>
                ...
                <p className="text-lg text-zinc-600 leading-relaxed">
                  ...
                  shouldn't feel mechanical—it should feel like an extension of
        ...
        <section
          id="view-colors"
          className={`view-section space-y-16 animate-in fade-in duration-700 ${view === "colors" ? "block" : "hidden"}`}
        >
          <div className="space-y-6 max-w-2xl">
            <h2 className="text-4xl md:text-5xl font-medium tracking-tight">
              Color System
